function [success,message] = xlswrite1(file,data,sheet,range)
...
try
    ...
    if ~isempty(file)
        if ~ischar(file)
            error('MATLAB:xlswrite1:InputClass','Filename must be a string');
        end
        % check for wildcards in filename
        if any(findstr('*', file))
            error('MATLAB:xlswrite1:FileName', 'Filename must not contain *');
        end
        ...
        if a1 && ~(a2.UserWrite == 1)
            error('MATLAB:xlswrite1:FileReadOnly', 'File can not be read only.');
        end
    else % get workbook filename.
        error('MATLAB:xlswrite1:EmptyFileName','Filename is empty.');
    end

    % Check for empty input data
    if isempty(data)
        error('MATLAB:xlswrite1:EmptyInput','Input array is empty.');
    end

    % Check for N-D array input data
    if ndims(data)>2
        error('MATLAB:xlswrite1:InputDimension',...
    ...
    if ~(iscell(data) || isnumeric(data) || ischar(data)) && ~islogical(data)
        error('MATLAB:xlswrite1:InputClass',...
    ...
    if ~isempty(nargchk(2,4,nargin))
        error('MATLAB:xlswrite1:InputArguments',nargchk(2,4,nargin));
    elseif nargout == 0
...
try
    ...
    try % select region.
        ...
        error('MATLAB:xlswrite1:SelectDataRange',lasterr);
    end
...
try
    ...
    warning('MATLAB:xlswrite1:AddSheet','Added specified worksheet.');
    if nargout > 0
...
try
    ...
    error('MATLAB:xlswrite1:CalculateRange',...
...
if d ~= floor(d) | any(d < 0) | any(d > 1/eps)
    error('MATLAB:xlswrite1:Dec2BaseInput',...

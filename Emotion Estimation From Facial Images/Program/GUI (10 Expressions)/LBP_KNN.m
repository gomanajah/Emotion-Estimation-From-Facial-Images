function [Sheet_Na,Angry,Contempt,Disgusted,Embarrass,Fear,Happy,Neutral,Pride,Sad,Surprised,Total,knn_Result,Test_Class,False_Expr] = LBP_KNN(CS)
clc
...
[Train_Angry_Matrix,Train_No_Of_AN_Images] = LBP(CS,Train_Angry_Folder,'Loading Angry Expression...','Extract HOG Features for Training set');[Train_Contempt_Matrix,Train_No_Of_CO_Images] = LBP(CS,Train_Contempt_Folder,'Loading Contempt Expression...','Extract HOG Features for Training set');[Train_Disgusted_Matrix,Train_No_Of_DI_Images] = LBP(CS,Train_Disgusted_Folder,'Loading Disgusted Expression...','Extract HOG Features for Training set');[Train_Embarrass_Matrix,Train_No_Of_EM_Images] = LBP(CS,Train_Embarrass_Folder,'Loading Embarrass Expression...','Extract HOG Features for Training set');[Train_Fear_Matrix,Train_No_Of_FE_Images] = LBP(CS,Train_Fear_Folder,'Loading Fear Expression...','Extract HOG Features for Training set');[Train_Happy_Matrix,Train_No_Of_HA_Images] = LBP(CS,Train_Happy_Folder,'Loading Happy Expression...','Extract HOG Features for Training set');[Train_neutral_Matrix,Train_No_Of_NE_Images] = LBP(CS,Train_Neutral_Folder,'Loading Neutral Expression...','Extract HOG Features for Training set');[Train_Pride_Matrix,Train_No_Of_PR_Images] = LBP(CS,Train_Pride_Folder,'Loading Pride Expression...','Extract HOG Features for Training set');[Train_Sad_Matrix,Train_No_Of_SA_Images] = LBP(CS,Train_Sad_Folder,'Loading Sad Expression...','Extract HOG Features for Training set');[Train_Surprised_Matrix,Train_No_Of_SU_Images] = LBP(CS,Train_Surprised_Folder,'Loading Surprised Expression...','Extract HOG Features for Training set');
%  ***********************
[Test_Angry_Matrix,Test_No_Of_AN_Images] = LBP(CS,Test_Angry_Folder,'Loading Angry Expression...','Extract HOG Features for Test set');[Test_Contempt_Matrix,Test_No_Of_CO_Images] = LBP(CS,Test_Contempt_Folder,'Loading Contempt Expression...','Extract HOG Features for Test set');[Test_Disgusted_Matrix,Test_No_Of_DI_Images] = LBP(CS,Test_Disgusted_Folder,'Loading Disgusted Expression...','Extract HOG Features for Test set');[Test_Embarrass_Matrix,Test_No_Of_EM_Images] = LBP(CS,Test_Embarrass_Folder,'Loading Embarrass Expression...','Extract HOG Features for Test set');[Test_Fear_Matrix,Test_No_Of_FE_Images] = LBP(CS,Test_Fear_Folder,'Loading Fear Expression...','Extract HOG Features for Test set');[Test_Happy_Matrix,Test_No_Of_HA_Images] = LBP(CS,Test_Happy_Folder,'Loading Happy Expression...','Extract HOG Features for Test set');[Test_neutral_Matrix,Test_No_Of_NE_Images] = LBP(CS,Test_Neutral_Folder,'Loading Neutral Expression...','Extract HOG Features for Test set');[Test_Pride_Matrix,Test_No_Of_PR_Images] = LBP(CS,Test_Pride_Folder,'Loading Pride Expression...','Extract HOG Features for Test set');[Test_Sad_Matrix,Test_No_Of_SA_Images] = LBP(CS,Test_Sad_Folder,'Loading Sad Expression...','Extract HOG Features for Test set');[Test_Surprised_Matrix,Test_No_Of_SU_Images] = LBP(CS,Test_Surprised_Folder,'Loading Surprised Expression...','Extract HOG Features for Test set');
%%  ************************ Integrating Matrices *************************
KNN_waitbar = waitbar(0.1,'Load Matrix...','name','wait...');%% KNN_waitbar
...
%%  ************************* KNN *****************************************
waitbar(0.2,KNN_waitbar,'KNN Classify...','name','wait...')%% KNN_waitbar
knn = ClassificationKNN.fit(Train_Matrix, Train_Class);
waitbar(0.7,KNN_waitbar,'KNN Predict...','name','wait...')%% KNN_waitbar
knn_Result = knn.predict(Test_Matrix);
close(KNN_waitbar)%% KNN_waitbar
...
for i=1:length(Test_Class)
     if (knn_Result(i)==Test_Class(i))
            if knn_Result(i) == 1
            AN=AN+1;
            elseif knn_Result(i) == 2
            CO=CO+1;
            elseif knn_Result(i) == 3
            DI=DI+1;
            elseif knn_Result(i) == 4
            EM=EM+1;
            elseif knn_Result(i) == 5
            FE=FE+1;
            elseif knn_Result(i) == 6
            HA=HA+1;
            elseif knn_Result(i) == 7
            NE=NE+1;
            elseif knn_Result(i) == 8
            PR=PR+1;
            elseif knn_Result(i) == 9
            SA=SA+1;
            elseif knn_Result(i) == 10
            SU=SU+1;
            end
         Result(i)=1;
